...
\section{Data Logger Shield}
	...
	Just like on your Arduino the shield pins are all labeled. But notice that one of our data logger shield types some of the pins are labeled with a white square and a back number. These pins (A4, A5, 9, 11, 12 and 13) are the pins that the shield uses for its functions. Make sure that your experiment doesn't use these pins. Pins A4 and A5 are used for the RTC and pins 9, 11, 12 and 13 are used for the SD card. The bottom of shield gives additional labels to these pins that describe their function.
...
And one more odd thing, there is a whole bunch of code at the end after the loop! These are additional functions that the loop and setup parts of the sketch use.  In python, new functions have to come at the beginning of the code because the python interpreter reads the code from the top and won't know about the functions if they come later. But our Arduino code is a variation of C++ and it is compiled before it is sent to the Arduino microcontroller. The compiler looks at all the code before it turns it into microcontroller instructions. So it will find the new functions and use them even though they are at the end of the code file.  You could make the temperature algorithm a new function at the end and just use it in the loop if you wish!

You will have to modify the example code. One strategy might be to start with the example code so we are sure we have all of these new parts included. Then add things one at a time to see if the new code breaks the existing code. 

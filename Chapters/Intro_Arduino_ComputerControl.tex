...
You should have one in your Arduino kit. Your instructor will have several
that you can borrow if you need another one.

\begin{figure}[h!]
	\centering
	\includegraphics[width=2.5in,height=.8in]{PH4CAU02}
\end{figure}
...
even ``breadboards.'' They are designed to allow you to put an electronic element on the board and then connect other things to that element. The last figure is a drawing of one of our proto-boards all on it's own. 
...
We placed one LED light wire in one column. On the other side of the board we place the other wire. The wires that come out of an electronic device are often called \textquotedblleft leads\textquotedblright\ and I will use this term. So one LED lead goes in a group of five holes on one side of the board and the other lead goes into a group of five hole on the other side of the board. 

If you look closely, you will see that one side of your LED is flat. The lead on the flat side should be toward the bottom (toward our $0\unit{V}$ connection that we will make). LED\ lights only work one direction. So when using LED's, if they don't work, turn them around.

Notice that the way we have done this one LED lead and one resistor lead are connected in one group of five holes. We need to choose a resistor. There are many in our kit. And if you look closely you will find that the resistors are glued together to strips of paper. And the paper is marked with a number! Choose one of the resistors that are marked with ``100.''

 The resistor is connected to the very bottom row. The top of the LED is not connected to anything yet. It is in a set of five holes that are connected together, but we need another wire to connect the top of the LED to the $+5\unit{V}.$ Let's choose the top of our power rows to be $+5\unit{V}$ and add the wire to connect the LED. 
...
This pin 13 is a digital output. That means it will either have $5\unit{V}$ or it will have $0\unit{V}.$ The $5\unit{V}$ will be our ``on'' value that could turn on an instrument. In our case it will turn on our LED\ light. The $0\unit{V}$ is the ``off'' value. When pin 13 has a value of $0\unit{V}$ our light will go off. We know that voltage is related to electrical energy. We will study voltage more next week. But for now we need to know that voltage is  always a comparison. So we need to know
...
Let me introduce the commands we will use. Each digital output has a number. The code defines a variable of type integer (int) and sets it equal to the pin number. That pin can be HIGH or LOW with HIGH $=+5\unit{V}$ and LOW $=0\unit{V}$. Each digital pin needs to be set up for output or input. This is done with a pinMode statement:
...
We will need to install this development environment. To do so we go to https://www.arduino.cc/en/Guide/HomePage and choose to instal the Arduino IDE for your computer. IDE stands for \textquotedblleft integrated development environment.\textquotedblright\ This is the environment that you see in the last figure. It has two unusual buttons on its toolbar. One is the ``compile'' button. When you used Python in PH150, you could run your code by pushing on a green arrow button in most development environments. 

If you used Jupyter scripts, they also had a function to run your code. But our Arduino is not big enough to be able to run code this way. We need to convert our human-understandable code that looks mostly like Python into a digital format that the Arduino can understand. The compile button does this. As the code is converted, the Arduino software looks for errors in the code. If there are errors, it will tell you at this point. This is different than Python which only told you about errors as the code ran. But remember we don't want to run our program on our computer. We want it to run on our Arduino. So this check is good before we send the translated code to the Arduino.
...
There are also some syntax differences between the Arduino computer language and Python. If you  know the ``C++'' language, you will recognize some of these changes.
...
Chances are you already have the Jupyter system because it comes standard as part of the Anaconda python system.  If you installed Anaconda, you can just go to your list of programs and start Jupyter. I am going to recommend you use Jupyter Lab instead of just Jupyter Notebook because it is easier to print your work from Jupyter lab. If you don't find it, call over your instructor or TA.  Here is how that looks in Windows 10 (For those with Macs, it will look more Macish). In Windows 11 you can start the Anaconda Navigator and launch Jupyter Lab from there.

\begin{figure}[h!]
	\centering
	\includegraphics[width=3.18353in,height=3.15674in]{Jupyter_start.jpg}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=3.8353in,height=1.15674in]{Jupyter_browser.jpg}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=3.8353in,height=1.15674in]{Jupyter_new_file.jpg}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=3.8353in,height=1.15674in]{Jupyter_input.jpg}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=3.8353in,height=1.15674in]{Jupyter_math.jpg}
\end{figure}
...
But at this point, you probably want to be able to submit your lab notebook for grading. This seems simple, but it's not. Jyputer Lab is a little easier, but it may ask you to install something (mine did). You just use the file/print and choose to print as a PDF. 

here is a video on how to make it work 

\begin{verbatim}
https://youtu.be/V5nl6vKM46U?si=daAEiLn1yTcIYs2x
\end{verbatim}

For Jupyter Notebooks we need to modify your Anaconda system to make it work well. To do this, start an Anaconda Powershell Prompt. In Windows 10 it you can find it like this.
 
\begin{figure}[h!]
	\centering
	\includegraphics[width=4.0704in,height=3.7228in]{Anaconda_Powershell.jpg}
\end{figure}

But in Windows 11 just type in Anaconda Powershell Prompt.  This opens a big black box. In the black box type the following lines.
...
\begin{figure}[h!]
	\centering
	\includegraphics[width=4.0704in,height=1.1228in]{Anaconda_pdf_mod.jpg}
\end{figure}

A lot of words and symbols will go by if you are successful.  After they are done you can try to save your Jupyter notebook as a pdf file for submission. From the File drop down menu choose ``Save and Export as'' and then ``PDF via HTML.'' You should find that your browser downloaded a PDF version of your notebook into your download directory.  This works for some systems.

\begin{figure}[h!]
	\centering
	\includegraphics[width=4.0704in,height=3.8228in]{Jupyter_pdf_output.jpg}
\end{figure}

Another approach is to save and export the notebook as HTML and then use MS Word or LibreOffice Writer to print it to a PDF file. I had more luck with this than with the fancy pyppeteer system.

Try this before you go on to the next part of the lab. Making a PDF file of your work might just be the hardest skill we learn today!  Make sure you can get a PDF file to submit.  If nothing works, go ahead and submit the .ipnyb file but call over your instructor or TA if you are having trouble with this.
